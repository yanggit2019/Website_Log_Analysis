...
--使用union all的方式进行行转列，把临时表的多行数据，转换一行
...
-- 9. sqoop脚本编写(导出hive中stats_view_depth_tmp表到MySQL的stats_view_depth表中)
sqoop export --connect jdbc:mysql://node0001:3306/report --username root --password 123 --table stats_view_depth --export-dir /hive/bigdater.db/stats_view_depth/*  --update-mode allowinsert --update-key platform_dimension_id,data_dimension_id,kpi_dimension_id
